...
def calc_spo2(raw_red, raw_ir, filt_red, filt_ir):
    ...
    dc_red = moving_average(np.array(raw_red), 0.5, SAMPLE_HZ)
    dc_ir  = moving_average(np.array(raw_ir),  0.5, SAMPLE_HZ)
    ...
    def ac_peak_to_peak(x, window_seconds, fs):
        ...
        return result / 2.0  # half-amplitude = amplitude equivalent to RMS normalisation

    ac_red = ac_peak_to_peak(np.array(filt_red), 0.5, SAMPLE_HZ)
    ac_ir  = ac_peak_to_peak(np.array(filt_ir),  0.5, SAMPLE_HZ)
...
def calc_hr(filt_ir):
    peaks, _ = signal.find_peaks(filt_ir, distance=10, prominence=0.75, height=np.mean(filt_ir)*1.25)
