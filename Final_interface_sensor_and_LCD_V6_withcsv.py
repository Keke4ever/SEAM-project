    ...
    with SMBus(BUSNUM) as bus:
        ...
        while not stop_evt.is_set():
            ...
            if n == 0:
                time.sleep(0.01)
                continue
            now_sec = time.monotonic() - start_time  
            for _ in range(n):
...
def processing_thread():
    ...
    start_time_calc = time.monotonic()
    while not stop_evt.is_set():
        ...
        now_sec = time.monotonic() - start_time_calc  

        with open('HR_andSPO2.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            
            header= ["Time", "Hr", "SPO2"]
            if f.tell() == 0:
                writer.writerow(header)
            data_point = [now_sec ,hr, spo2]
