...
SAMPLE_HZ = 50
WINDOW_SEC = 5             # processing window for HR/SpO2
...
_motion_state  = "STILL"
_motion_fall_detection = "NOT FALLING"
...
def set_motion(state: str, fall_detection: str):
    global _motion_state, _motion_fall_detection
    with _mot_lock:
        _motion_state  = state
        _motion_fall_detection=fall_detection


def get_motion():
    with _mot_lock:
        return _motion_state, _motion_fall_detection
 
# ---- Registers for PPG Sensor ----
...
### Functions for reading Data from PPG
...
def notify_callback(notifying, charcteristic):
    if notifying:
            async_tools.add_timer_seconds(2, update_value, charcteristic)
            print("just notifies")



### ----     Thread 1 for PPG Sensor ------
def sensor_thread():
        period = 1.0 / 100
        
        csv_file =open("sensor_data4.csv" , "w", newline= '')
        csv_writer =csv.writer(csv_file)
        csv_writer.writerow(['timestamps_ms', 'ir', 'red'])
        ...
        with SMBus(BUSNUM) as bus:
            ...
            time.sleep(0.02)
        csv_file.close()
        print("CSV file sensor_data saved")
...
def SPO2(Raw_red, Raw_IR,filtered_red,filtered_IR):
    ...
    dc_window_s = 1 # DC baseline window
    ac_window_s = 1 # AC RMS window (can be the same)
    fs=SAMPLE_HZ
...
def Heart_rate(filtered_red):
    ...
    #heart rate
    peaks_index, peak_heights=signal.find_peaks(filtered_red,  distance=20)
    
    hr = str((60/WINDOW_SEC)*len(peaks_index))
    print("Heart rate:" + str(hr))
    return hr
   
### --- Thread 3 IMU motion detection ---
def imu_thread():
    ...
    while not stop_evt.is_set():
        ...
        # Free-fall
        if raw_mag < FREEFALL_THRESHOLD and lin_mag > IMPACT_THRESHOLD:
            if not was_freefall:
                print(f"IS FALLING HELP")
                set_motion("MOVING", "FALLING" )
                was_freefall = True
        else:
            was_freefall = False

        # General motion
        in_motion = (lin_mag > MOTION_ACCEL_THRESHOLD)
        ...
        if in_motion:
            ...
            if not was_moving:
                ...
                was_moving = True
            if is_linear:
                set_motion("MOVING", "NOT FALLING")
            else:
                set_motion("MOVING", "NOT FALLING")
        elif was_moving and (now - last_motion_time) > STILL_DURATION:
            print("[IMU] STILL")
            set_motion("STILL", "NOT FALLING")
            was_moving = False
...
#Thread 3 Calculations and add to data queue
def processing_thread():
    ...
    while not stop_evt.is_set():
        ...
        filtered_IR = signal.filtfilt(b, a, ir_raw_buf)
        filtered_red=signal.filtfilt(b, a, red_raw_buf)
        hr = Heart_rate(filtered_red)
        spo2 = SPO2(red_raw_buf, ir_raw_buf,filtered_red,filtered_IR)
        Data=str(hr) + "," + str(spo2)
        data_queue.put(Data)
        print(str(len(ir_raw_buf)) + "This is window sample number")
        if len(ir_raw_buf) >=WINDOW_N:
            for i in range(50):
...
def main():
    ...
    t3=threading.Thread(target=peripheral_ble, daemon=True)
