...
def init_spo2(bus):
    ...
    write_reg(bus, REG_SPO2_CONFIG, 0x67)  # FIX: bits[6:5]=11 → full 16384 nA ADC range (was 0x27 = 4096 nA, too small for 7.2 mA LED current, caused clipping)
...
def ac_peak_to_peak(x, window_seconds, fs):
    """Sliding peak-to-peak AC amplitude. Correct AC metric for R ratio."""
    w = max(1, int(window_seconds * fs))
    result = np.zeros(len(x))
    for i in range(len(x)):
        lo = max(0, i - w // 2)
        hi = min(len(x), i + w // 2 + 1)
        result[i] = np.max(x[lo:hi]) - np.min(x[lo:hi])
    return result

def calc_spo2(raw_red, raw_ir, filt_red, filt_ir):
    
    
    # DC baseline: 1.5 s window — long enough to span one full pulse cycle at ~40 bpm
    dc_red = moving_average(raw_red, 1.5, SAMPLE_HZ )   
    dc_ir  = moving_average(raw_ir,  1.5, SAMPLE_HZ )   

    # AC amplitude: peak-to-peak over 1 s sliding window
    ac_red = ac_peak_to_peak(filt_red, 1.0, SAMPLE_HZ )  
    ac_ir  = ac_peak_to_peak(filt_ir,  1.0, SAMPLE_HZ )  
...
def calc_hr(filt_ir, time_buf):
    ...
    peaks, _ = signal.find_peaks(filt_ir, distance=10, prominence=0.3 * np.std(filt_ir))
    if len(peaks) < 2:
        return 0.0
    inter_peak = []
    for i in range(len(peaks) - 1):
        seconds = time_buf[peaks[i + 1]] - time_buf[peaks[i]]  
        if seconds > 0:
            inter_peak.append(seconds)
    if not inter_peak:
        return 0.0
    ...
    with SMBus(BUSNUM) as bus:
        ...
        while not stop_evt.is_set():
            ...
            if n == 0:
                time.sleep(0.01)
                continue
            now_sec = time.monotonic() - start_time  # BUG 3 FIX: keep float seconds, not int — int truncation made all IBIs 0 or 1 s
            for _ in range(n):
                red, ir = read_sample(bus)
                try:
                    raw_q.put_nowait((now_sec, ir, red))
                except queue.Full:
                    try:
                        raw_q.get_nowait()
                        raw_q.put_nowait((now_sec, ir, red))
...
def processing_thread():
    ...
    while not stop_evt.is_set():
        try:
            t_sec, ir, red = raw_q.get(timeout=0.25)  # t_sec is float seconds
            data_point = [t_sec, ir, red]
        except queue.Empty:
            continue

        ir_buf.append(ir); red_buf.append(red); time_buf.append(t_sec)
        if len(ir_buf) < WINDOW_N:
            continue

        filt_ir  = signal.filtfilt(b, a, ir_buf)
        filt_red = signal.filtfilt(b, a, red_buf)
        # FIX Bug 1: raw_q.put_nowait stores (t_ms, ir, red) — so ir_buf is IR and red_buf is RED.
        #            Original call had arguments transposed. Corrected order: red first, then ir.
        hr   = calc_hr(filt_ir,time_buf)
        ...
        if len(ir_buf) >= WINDOW_N:
            del ir_buf[:50]; del red_buf[:50]; del time_buf[:50]
