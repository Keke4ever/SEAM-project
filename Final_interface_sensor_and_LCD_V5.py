...
def calc_hr(filt_ir, time ):
    peaks, peak_index = signal.find_peaks(filt_ir, distance=10, prominence=0.3 * np.std(filt_ir))
    inter_peak=[]
    for i in range (0,len(peak_index)-2):
        seconds=time[peak_index+1]-time[peak_index]
        inter_peak.append(seconds)
    avg_inter_peak=np.mean(np.array(inter_peak))
    hr = (60 / avg_inter_peak)
...
def processing_thread():
    ...
    while not stop_evt.is_set():
        ...
        ir_buf.append(ir); red_buf.append(red); time_buf.append(t_ms)
