...
def calc_spo2(raw_red, raw_ir, filt_red, filt_ir):
    dc_red = moving_average(np.array(raw_red),  2, SAMPLE_HZ)
    dc_ir  = moving_average(np.array(raw_ir),   2, SAMPLE_HZ)
    ac_red = running_rms(np.array(filt_red),     2, SAMPLE_HZ)
    ac_ir  = running_rms(np.array(filt_ir),      2, SAMPLE_HZ)
    eps    = 1e-8
