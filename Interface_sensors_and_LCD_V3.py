...
def processing_thread():
    ...
    while not stop_evt.is_set():
        ...
        mot_state, fall_detection = get_motion()
        payload = f"{int(hr)},{int(spo2)},{mot_state},{fall_detection}"
...
is_connected   = False
client_address = None
_notify_active = False          # guards against stacked timers
...
def on_disconnect(adp, device):
    ...
    is_connected   = False
    client_address = None
    _notify_active = False      # reset so next connection gets a fresh timer
...
def get_latest_payload():
    ...
    try:
        ...
        if latest is not None:
            ...
            # No HR/SpO2 yet — send current motion state
            mot_state, fall_detection = get_motion()
            message = f"0,0,{mot_state},{fall_detection}"

        return list(message.encode("utf-8"))
    except Exception as e:
        print(f"[BLE] get_latest_payload error: {e}")
        return list("0,0,STILL,NOT FALLING".encode("utf-8"))
...
def update_value(characteristic):
    """Called every 2 s by the async timer. Must return True to keep firing."""
    if not is_connected:
        return False            # stop the timer cleanly; next connect re-arms it
    try:
        characteristic.set_value(get_latest_payload())
    except Exception as e:
        print(f"[BLE] update_value error: {e}")
    return True                 # always reschedule while connected
